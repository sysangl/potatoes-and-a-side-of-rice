...
cacheDir="${HOME}/.cache/wallpicker/$(cat ~/.current_rice_theme)"
rofi_command="rofi -dmenu -theme ${HOME}/.config/rofi/wall-select.rasi -theme-str ${rofi_override}"
	...
	if [ -f "$imagen" ]; then
		file_name=$(basename "$imagen")
			if [ ! -f "${cacheDir}/${file_name}" ] ; then
				convert -strip "$imagen" -thumbnail 400x400^ -gravity center -extent 400x400 "${cacheDir}/${file_name}"
			fi
    fi
done

# Select a picture with rofi
wall_selection=$(find "${wall_dir}"  -maxdepth 1  -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.webp" \) -exec basename {} \; | sort | while read -r A ; do  echo -en "$A""${cacheDir}"/"$A\n" ; done | $rofi_command)

# Set the wallpaper
[[ -n "$wall_selection" ]] || exit 1
$HOME/.config/hypr/scripts/change-wallpaper.sh ${wall_selection}
